...
1. Skompiluj projekt przy użyciu `npm run build`.
2. Uruchom serwer (np. `node dist/backend/index.js`).
3. Frontend znajduje się w katalogu `frontend/` – dostęp do plików statycznych przez endpoint `/static/`.
4. Kod źródłowy frontendu w TypeScript znajduje się w `frontend/src/`, a skompilowane moduły trafiają do `frontend/dist/frontend/src/`.
